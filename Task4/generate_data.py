...
Produces four CSV files in the Task4/ directory:
  - network_traffic.csv
  - endpoints.csv
  - backups.csv
  - staff_readiness.csv
...
def random_ip(private: bool = False) -> str:
    ...
    Args:
        private: If True, generate a RFC-1918 private address (10.x.x.x).
                 If False, generate a public-looking address.
...
# Real-world city coordinates used for geographic variety on the map chart.
CITY_COORDS = {
...
def generate_network_traffic(n: int = 10) -> pd.DataFrame:
    """Generate dummy network traffic records.

    Each row represents one captured network session with realistic
    protocol distribution, throughput values, and geographic attribution.
    ...
    for i in range(n):
        ...
        anomaly = 1 if random.random() < 0.25 else 0   # ~25 % anomaly rate
        ...
        rows.append({
            ...
            "src_lat":              lat,
            "src_lon":              lon,
            "dst_country":          "GB",           # financial org is UK-based
        })
...
def generate_endpoints(n: int = 10) -> pd.DataFrame:
    """Generate dummy endpoint security status records.

    Covers workstations and servers with varied OS, security tool,
    and patch-level states to demonstrate the dashboard's endpoint panel.
    ...
    rows = []
    for i in range(n):
        fw_active = random.random() > 0.15                   # 85 % active
        av_status = random.choice(av_statuses)
...
def generate_backups(n: int = 10) -> pd.DataFrame:
    """Generate dummy backup status records.

    Covers a variety of systems with realistic coverage percentages,
    encryption states, and RTO compliance flags.
    ...
    for i in range(n):
        ...
        validated = "Pass" if random.random() > 0.2 else "Fail"
...
def generate_staff_readiness(n: int = 10) -> pd.DataFrame:
    """Generate dummy staff security-readiness records.

    Includes department, role, training completion, and phishing
    simulation results to populate the Staff Readiness dashboard panel.

    Args:
        n: Number of rows to generate.

    Returns:
        DataFrame with staff readiness data.
    """
    departments = ["Finance", "IT", "Operations", "HR", "Compliance",
                   "Risk", "Trading", "Legal", "Customer Services", "Executive"]
    roles = [
        "Analyst", "Manager", "Director", "Developer",
        "Administrator", "Officer", "Trader", "Advisor",
    ]
    risk_profiles = ["Low", "Low", "Medium", "Medium", "High"]   # weighted

    rows = []
    for i in range(n):
        completion = round(random.uniform(20, 100), 1)
        risk       = random.choice(risk_profiles)
        phish_pass = "Pass" if (risk == "Low" and random.random() > 0.1) or \
                               (risk == "Medium" and random.random() > 0.35) or \
                               (risk == "High" and random.random() > 0.55) else "Fail"
...
def main() -> None:
    """Generate all four CSV data files and save them to the Task4 directory."""

    datasets = {
        "network_traffic.csv": generate_network_traffic(10),
        "endpoints.csv":       generate_endpoints(10),
        "backups.csv":         generate_backups(10),
        "staff_readiness.csv": generate_staff_readiness(10),
    }
